<template>
    <div>
        <div v-if="!contentReady">
            <div v-if="!vihKeys.length" class="d-flex justify-content-center">
        ...
        <div class="card mt-2">
            ...
            <div class="card-body table-responsive">
                <table class="table table-hover table-sm">
                    ...
                    <tbody v-for="vihKey in setPaginate" :key="vihKey.id">
                        <th scope="row">{{ vihKey.id }}</th>
                        <td>{{ vihKey.description }}</td>
                        <td>
                            <span
                                :class="
                                    vihKey.state.id === 1
                                ...
                                "
                            >
                                {{ vihKey.state.description }}</span
                            >
                        </td>
                        <td class="text-center py-1 align-middle">
                            <div class="btn-group btn-group-sm">
                                <button
                                    @click.prevent="setEdit(vihKey)"
...
export default {
    ...
    computed: {
        filterData() {
            const filtered = this.vihKeys.filter(vihKey => {
                return vihKey.description
                    .toLowerCase()
    ...
    methods: {
            ...
            if (this.validateInput()) {
                ...
                try {
                    ...
                    if (response.status === 200) {
                        toast.fire({
                            icon: "success",
                            title: "Opción clave VIH creada exitosamente"
                        });
                        this.vihKeys.push(response.data.vihKey);
                        this.resetForm();
                    }
                } catch (e) {
                    console.log(e);
        ...
        async edit() {
            ...
            try {
                ...
                if (response.status === 200) {
                    const index = this.vihKeys.findIndex(
                        find => find.id === response.data.vihKey.id
                    );

                    toast.fire({
                        icon: "success",
                        title: "Opción clave VIH editada exitosamente"
                    });

                    this.vihKeys.splice(index, 1, response.data.vihKey);
                    this.resetForm();
                }
            } catch (e) {
                console.log(e);
        ...
        setEdit(vihKey) {
            ...
            this.description = vihKey.description;
            this.selectedState = vihKey.state.id;
            this.id = vihKey.id;
        },
        async destroy(vihKey) {
            const confirmation = await swal.fire({
                title: "¿Estás seguro?",
                text: "La opción clave VIH se eliminará permanentemente",
                icon: "warning",
            ...
            if (confirmation.value) {
                try {
                    ...
                    if (response.status === 200) {
                        toast.fire({
                            icon: "success",
                            title: "Opción clave VIH eliminada"
                        });
                        const index = this.vihKeys.findIndex(
                            find => find.id === vihKey.id
                        );
                        this.vihKeys.splice(index, 1);
                    }
                } catch (e) {
                    console.log(e);
                }
