        ...
        <div class="card mt-2">
            ...
            <div class="card-body table-responsive">
                <table class="table table-hover table-sm">
                    ...
                    <tbody
                        v-for="quantitySample in setPaginate"
                        :key="quantitySample.id"
                    >
                        <th scope="row">{{ quantitySample.id }}</th>
                        <td>{{ quantitySample.description }}</td>
                        <td>
                            <span
                                :class="
                                    quantitySample.state.id === 1
                                        ? 'badge badge-success'
                                        : 'badge badge-danger'
                                "
                            >
                                {{ quantitySample.state.description }}</span
                            >
                        </td>
                        <td class="text-center py-1 align-middle">
                            <div class="btn-group btn-group-sm">
                                <button
                                    @click.prevent="setEdit(quantitySample)"
                                ...
                                <button
                                    ...
                                    @click.prevent="destroy(quantitySample)"
                                >
...
export default {
    ...
    computed: {
        filterData() {
            const filtered = this.quantitySamples.filter(quantitySample => {
                return quantitySample.description
                    .toLowerCase()
    ...
    methods: {
        async getQuantitySamples() {
            try {
                const response = await fetch('/api/quantity-sample')
                const json = await response.json()

                console.log(json)
                this.quantitySamples = json.quantitySamples
            ...
            if (this.validateInput()) {
                ...
                try {
                    ...
                    if (response.status === 200) {
                        ...
                        this.quantitySamples.push(response.data.quantitySample)
                        this.resetForm()
                    }
                } catch (e) {
                    console.log(e)
        ...
        async edit() {
            ...
            try {
                const response = await axios.patch(
                    `/api/quantity-sample/${this.id}`,
                    params
                )

                if (response.status === 200) {
                    const index = this.quantitySamples.findIndex(
                        find => find.id === response.data.quantitySample.id
                    )
                    ...
                    this.quantitySamples.splice(
                        index,
                        1,
                        response.data.quantitySample
                    )
                    this.resetForm()
                }
            } catch (e) {
        ...
        setEdit(quantitySample) {
            ...
            this.description = quantitySample.description
            this.selectedState = quantitySample.state.id
            this.id = quantitySample.id
        },
        async destroy(quantitySample) {
            ...
            if (confirmation.value) {
                try {
                    const response = await axios.delete(
                        `/api/quantity-sample/${quantitySample.id}`
                    )
                    ...
                    if (response.status === 200) {
                        ...
                        const index = this.quantitySamples.findIndex(
                            find => find.id === quantitySample.id
                        )
                        this.quantitySamples.splice(index, 1)
                    }
                } catch (e) {
                    console.log(e)
                }
