<template>
    <div>
        <div v-if="!contentReady">
            <div v-if="!availables.length" class="d-flex justify-content-center">
        ...
        <div class="card mt-2">
            ...
            <div class="card-body table-responsive">
                <table class="table table-hover table-sm">
                    ...
                    <tbody v-for="available in setPaginate" :key="available.id">
                        <th scope="row">{{ available.id }}</th>
                        <td>{{ available.description }}</td>
                        <td>
                            <span
                                :class="
                                    available.state.id === 1
                                        ? 'badge badge-success'
                                        : 'badge badge-danger'
                                "
                            >
                                {{ available.state.description }}</span
                            >
                        </td>
                        <td class="text-center py-1 align-middle">
                            <div class="btn-group btn-group-sm">
                                <button
                                    @click.prevent="setEdit(available)"
                                ...
                                <button
                                    ...
                                    @click.prevent="destroy(available)"
                                >
...
export default {
    ...
    computed: {
        filterData() {
            const filtered = this.availables.filter(available => {
    ...
    methods: {
        ...
        async save() {
            if (this.validateInput()) {
                ...
                try {
                    const response = await axios.post("/api/available", params);

                    if (response.status === 200) {
                        toast.fire({
                            icon: "success",
                            title: "Disponibilidad creada exitosamente"
                        });
                        this.availables.push(response.data.available);
                        this.resetForm();
                    }
                } catch (e) {
                    console.log(e);
        ...
        async edit() {
            ...
            try {
                const response = await axios.patch(
                    `/api/available/${this.id}`,
                    params
                );

                if (response.status === 200) {
                    const index = this.availables.findIndex(
                        find => find.id === response.data.available.id
                    );

                    toast.fire({
                        icon: "success",
                        title: "Disponibilidad editada exitosamente"
                    });

                    this.availables.splice(index, 1, response.data.available);
                    this.resetForm();
                }
            } catch (e) {
                console.log(e);
        ...
        setEdit(available) {
            ...
            this.selectedState = available.state.id;
            this.id = available.id;
        },
        async destroy(available) {
