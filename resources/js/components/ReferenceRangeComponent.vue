                ...
                <form role="form">
                    <div class="card-body">
                        ...
                        <div class="row">
                            ...
                            <div class="col-sm-4 col-md-4 col-4">
                                <label>Rango referencia crítico </label>
                            </div>
                        </div>
                        <div class="row">
                            ...
                            <div class="col-sm-4 col-md-4 col-4 form-group">
                                <input
                                    v-model="normalMinimun"
                                    type="text"
                                    class="form-control mb-1"
                                    placeholder="Valor mínimo"
                                />
                                <input
                                    v-model="normalMaximum"
                                    type="text"
                                    class="form-control"
                            ...
                            <div class="col-sm-4 col-md-4 col-4 form-group">
                                <input
                                    v-model="criticalMinimun"
                                    type="text"
                                    class="form-control mb-1"
                                    placeholder="Valor mínimo"
                                />
                                <input
                                    v-model="criticalMaximum"
                                    type="text"
                                    class="form-control"
                                    placeholder="Valor máximo"
                                />
                    ...
                    <div class="card-body table-responsive">
                        <table class="table table-hover table-sm">
                            ...
                            <tbody v-for="(item, index) in setPaginate">
                                ...
                                <td>
                                    {{ item.age_start }} a {{ item.age_end }}
                                    {{ item.age_unit.description }}
                                </td>
                                <td v-if="item.type_value === 'CUANTITATIVO'">
                                    {{ item.normal_minimum }} a
                                    {{ item.normal_maximum }}
                                    {{ item.test.unit.description }}
                                </td>
                                <td v-else>-</td>
                                <td v-if="item.type_value === 'CUANTITATIVO'">
                                    <strong>Mínimo: </strong
                                    >{{ item.critical_minimum }}
                                    {{ item.test.unit.description }}
                                    <br /><strong>Máximo: </strong
                                    >{{ item.critical_maximum }}
                                    {{ item.test.unit.description }}
                                </td>
                                <td v-else>-</td>
                                <td v-if="item.type_value === 'CUANTITATIVO'">
...
export default {
    ...
    computed: {
        filterData() {
            const filtered = this.referenceRanges.filter(referenceRange => {
                return referenceRange.test.description
                    .toLowerCase()
    ...
    methods: {
        setValueResult(selected) {
            if (selected.typeValue === 'CUALITATIVO') return selected
        },
        startProgressiveBar() {
            let width = 0
            const vm = this
            ...
            if (this.validateInput()) {
                ...
                try {
                    const response = await axios.put(
                        `/api/referenceRange/${this.id}`,
                        params
                    )

                    if (response.status === 200) {
                        const index = this.referenceRanges.findIndex(
                            find => find.id === response.data.referenceRange.id
                        )
                        console.log(index)
                        toast.fire({
                            icon: 'success',
                        ...
                        this.referenceRanges.splice(
                            index,
                            1,
                            response.data.referenceRange
                        )
                        this.resetForm()
                    }
                } catch (e) {
