<template>
    <div>
        <div v-if="!contentReady">
            <div v-if="!methods.length" class="d-flex justify-content-center">
        ...
        <div class="card mt-2">
            ...
            <div class="card-body table-responsive">
                <table class="table table-hover table-sm">
                    ...
                    <tbody v-for="method in setPaginate" :key="method.id">
                        ...
                        <td>
                            <span
                                ...
                                "
                            >
                                {{ method.state.description }}</span
                            >
                        </td>
                        <td class="text-center py-1 align-middle">
                            <div class="btn-group btn-group-sm">
                                <button
                                    @click.prevent="setEdit(method)"
...
export default {
    ...
    computed: {
        filterData() {
            const filtered = this.methods.filter(method => {
    ...
    methods: {
            ...
            if (this.validateInput()) {
                ...
                try {
                    ...
                    if (response.status === 200) {
                        toast.fire({
                            icon: 'success',
                            title: 'Método creado exitosamente'
                        })
                        this.methods.push(response.data.method)
                        this.resetForm()
                    }
                } catch (e) {
                    console.log(e)
            ...
            try {
                ...
                if (response.status === 200) {
                    const index = this.methods.findIndex(
                        find => find.id === response.data.method.id
                    )

                    toast.fire({
                        icon: 'success',
                        title: 'Método editado exitosamente'
                    })

                    this.methods.splice(index, 1, response.data.method)
                    this.resetForm()
                }
            } catch (e) {
                console.log(e)
        ...
        setEdit(method) {
            ...
            this.selectedState = method.state.id
            this.id = method.id
        },
        async destroy(method) {
            const confirmation = await swal.fire({
                title: '¿Estás seguro?',
                text: 'El método se eliminará permanentemente',
                icon: 'warning',
