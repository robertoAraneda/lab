        ...
        <div class="card mt-2">
            ...
            <div class="card-body table-responsive">
                <table class="table table-hover table-sm">
                    ...
                    <tbody
                        v-for="sampleCondition in setPaginate"
                        :key="sampleCondition.id"
                    >
                        <th scope="row">{{ sampleCondition.id }}</th>
                        <td>{{ sampleCondition.description }}</td>
                        <td>
                            <span
                                :class="
                                    sampleCondition.state.id === 1
                                        ? 'badge badge-success'
                                        : 'badge badge-danger'
                                "
                            >
                                {{ sampleCondition.state.description }}</span
                            >
                        </td>
                        <td class="text-center py-1 align-middle">
                            <div class="btn-group btn-group-sm">
                                <button
                                    @click.prevent="setEdit(sampleCondition)"
                                ...
                                <button
                                    ...
                                    @click.prevent="destroy(sampleCondition)"
                                >
...
export default {
    ...
    computed: {
        filterData() {
            const filtered = this.sampleConditions.filter(sampleCondition => {
                return sampleCondition.description
                    .toLowerCase()
    ...
    methods: {
        async getSampleConditions() {
            try {
                const response = await fetch('/api/sampleCondition')
                const json = await response.json()

                this.sampleConditions = json.sampleConditions
                ...
                try {
                    ...
                    if (response.status === 200) {
                        toast.fire({
                            icon: 'success',
                            title:
                                'Condición de toma de muestra creada exitosamente'
                        })
                        this.sampleConditions.push(
                            response.data.sampleCondition
                        )
                        this.resetForm()
                    }
                } catch (e) {
        ...
        async edit() {
            ...
            try {
                const response = await axios.patch(
                    `/api/sampleCondition/${this.id}`,
                    params
                )

                if (response.status === 200) {
                    const index = this.sampleConditions.findIndex(
                        find => find.id === response.data.sampleCondition.id
                    )

                    toast.fire({
                        icon: 'success',
                        title:
                            'Condición de toma de muestra editada exitosamente'
                    })

                    this.sampleConditions.splice(
                        index,
                        1,
                        response.data.sampleCondition
                    )
        ...
        setEdit(sampleCondition) {
            ...
            this.description = sampleCondition.description
            this.selectedState = sampleCondition.state.id
            this.id = sampleCondition.id
        },
        async destroy(sampleCondition) {
            ...
            if (confirmation.value) {
                try {
                    const response = await axios.delete(
                        `/api/sampleCondition/${sampleCondition.id}`
                    )
