                ...
                <div class="card-header bg-secondary">
                    ...
                    <div class="card-tools">
                        <button v-if="!editing" @click="setFormContent" type="button"
                                class="btn btn-success float-right" data-toggle="tooltip" data-placement="top" title="CREAR REGISTRO">
        ...
        computed: {
            filterData() {
                const filtered = this.ageUnits.filter((ageUnit) => {
                    return ageUnit.description.toLowerCase().match(this.search_item.toLowerCase());
        ...
        methods: {
            ...
            async edit() {
                if (this.validateInput()) {
                    ...
                    try {
                        const response = await axios.put(`/api/ageUnit/${this.id}`, params)

                        if (response.status === 200) {
                            const index = this.ageUnits.findIndex(find => find.id === response.data.ageUnit.id)
                            console.log(index)
                            toast.fire({
                                icon: 'success',
            ...
            async destroy(item, index) {
                ...
                if (confirmation.value) {
                    try {
                        const response = await axios.delete(`/api/ageUnit/${item.id}`);
