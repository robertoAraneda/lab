<template>
    <div>
        <div v-if="!contentReady">
            <div v-if="!units.length" class="d-flex justify-content-center">
        ...
        <div class="card mt-2">
            ...
            <div class="card-body table-responsive">
                <table class="table table-hover table-sm">
                    ...
                    <tbody v-for="unit in setPaginate" :key="unit.id">
                        ...
                        <td>
                            <span
                                ...
                                "
                            >
                                {{ unit.state.description }}</span
                            >
                        </td>
                        <td class="text-center py-1 align-middle">
                            <div class="btn-group btn-group-sm">
                                <button
                                    @click.prevent="setEdit(unit)"
    ...
    methods: {
        ...
        async save() {
            if (this.validateInput()) {
                ...
                try {
                    const response = await axios.post('/api/unit', params)
            ...
            try {
                ...
                if (response.status === 200) {
                    const index = this.units.findIndex(
                        find => find.id === response.data.unit.id
                    )

                    toast.fire({
                        icon: 'success',
                        title: 'Unidad editada exitosamente'
                    })

                    this.units.splice(index, 1, response.data.unit)
                    this.resetForm()
                }
            } catch (e) {
                console.log(e)
        ...
        setEdit(unit) {
            ...
            this.selectedState = unit.state.id
            this.id = unit.id
        },
        async destroy(unit) {
            const confirmation = await swal.fire({
                title: '¿Estás seguro?',
                text: 'La unidad se eliminará permanentemente',
                icon: 'warning',
