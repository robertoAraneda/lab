<template>
    <div>
        <div v-if="!contentReady">
            <div v-if="!workareas.length" class="d-flex justify-content-center">
        ...
        <div class="card mt-2">
            ...
            <div class="card-body table-responsive">
                <table class="table table-hover table-sm">
                    ...
                    <tbody v-for="workarea in setPaginate" :key="workarea.id">
                        <th scope="row">{{ workarea.id }}</th>
                        <td>{{ workarea.description }}</td>
                        <td>{{ workarea.section.description }}</td>
                        <td>{{ workarea.chief_user.name }}</td>
                        <td>
                            <span
                                :class="
                                    workarea.state.id === 1
                                ...
                                "
                            >
                                {{ workarea.state.description }}</span
                            >
                        </td>
    ...
    methods: {
        ...
        async getSections() {
            try {
                ...
                this.sections = this.parseSelect(json.sections);
        ...
        async getUsers() {
            try {
                ...
                this.users = json.users.map(function(user) {
                    return {
                        id: user.id,
                        text: user.name
                    };
        ...
        async save() {
            if (this.validateInput()) {
                let params = {
                    ...
                    chief_user_id: this.selectedUser
                };
                try {
                    const response = await axios.post("/api/workarea", params);

                    if (response.status === 200) {
                        toast.fire({
                            icon: "success",
                            title: "Área de trabajo creada exitosamente"
                        });
                        this.workareas.push(response.data.workarea);
                        this.resetForm();
                    }
                } catch (e) {
                    console.log(e);
        ...
        async edit() {
            ...
            try {
                const response = await axios.patch(
                    `/api/workarea/${this.id}`,
                    params
                );

                if (response.status === 200) {
                    const index = this.workareas.findIndex(
                        find => find.id === response.data.workarea.id
                    );

                    toast.fire({
                        icon: "success",
                        title: "Área de trabajo editada exitosamente"
                    });

                    this.workareas.splice(index, 1, response.data.workarea);
                    this.resetForm();
                }
            } catch (e) {
                console.log(e);
        ...
        setEdit(workarea) {
            ...
            this.description = workarea.description;
            this.selectedState = workarea.state.id;
            this.selectedSection = workarea.section.id;
            this.selectedUser = workarea.chief_user.id;
            this.id = workarea.id;
        },
        async destroy(workarea) {
