...
export default {
    ...
    computed: {
        filterData() {
            const filtered = this.ubications.filter(ubication => {
                return ubication.description
                    .toLowerCase()
        ...
        async save() {
            if (this.validateInput()) {
                ...
                try {
                    ...
                    const url = '/api/store/ubications'
                    ...
                    if (response.status >= 200 && response.status < 300) {
                        ...
                        this.ubications.push(json.ubication)
                        this.resetForm()
                    } else {
            ...
            if (this.validateInput()) {
                ...
                try {
                    const response = await axios.put(
                        `/api/store/ubications/${this.id}`,
                        params
                    )

                    if (response.status === 200) {
                        const index = this.ubications.findIndex(
                            find => find.id === response.data.ubication.id
                        )
                        console.log(index)
                        toast.fire({
                            icon: 'success',
                        ...
                        this.ubications.splice(
                            index,
                            1,
                            response.data.ubication
                        )
                        this.resetForm()
                    }
                } catch (e) {
            ...
            if (confirmation.value) {
                try {
                    ...
                    if (response.status === 200) {
                        ...
                        this.ubications.splice(index, 1)
                    }
                } catch (e) {
                    console.log(e)
                }
